...
class Music(Cog, wavelink.WavelinkMixin):
	...
	async def song_lyrics(self, ctx, title):
		lyrics_url = f"https://some-random-api.ml/lyrics?title={title}"

		async with request("GET", lyrics_url, headers=[]) as response:
			...
			for chunk in [ly[i : i + 2000] for i in range(0, len(ly), 2000)]:

				embed = discord.Embed(
					title=f"**{(str(sn))} by {(str(au))}**",
					description=chunk,
					color = 0x000000,
				)
				embed.set_footer(
