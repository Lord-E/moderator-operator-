...
from discord import Activity, ActivityType, Embed, activity, client
...
class Meta(Cog):
	def __init__(self, bot):
		...
		self._message = "playing op.help | {users:,} users in {guilds:,} servers"
	...
	async def show_bot_stats(self, ctx):
		...
		fields = [
			("Bot name", self.bot.VERSION, True),
			("Python version", python_version(), True),
			("discord.py version", discord_version, True),
			("Uptime", uptime, True),
			("CPU time", cpu_time, True),
			("Memory usage", f"{mem_usage:,.3f} / {mem_total:,.0f} MB ({mem_of_total:.0f}%)", True),
